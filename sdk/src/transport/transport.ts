...
import type { SendResult } from "./retry-queue";
...
export class Transport {
  ...
  constructor(
    ...
    this.retry = new RetryQueue(
      MAX_ATTEMPTS,
      (item) => this.post(item.kind, item.body),
      (item) => {
        this.offline.add({
          id: uuid(),
          kind: item.kind,
  ...
  private async post(kind: PayloadKind, body: string): Promise<SendResult> {
