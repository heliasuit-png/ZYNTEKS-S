      ...
      <div className="mt-3 space-y-2 rounded-lg border border-zt-border/60 bg-white/[0.015] p-3">
        <div className="flex items-center justify-between">
          <span className="text-[11px] font-medium uppercase tracking-wide text-zt-muted">
            {dict.dash.insights.aiConfidence}
          </span>
          <ConfidenceBar value={rec.confidence} />
        </div>
        <p className="text-xs text-zt-muted">
          <span className="font-medium text-zt-text">{dict.dash.insights.reasoning} </span>
          {rec.reasoning}
        </p>
          ...
          {correlation.events.map((ev, i) => (
            <li key={`${correlation.id}-ev-${i}`} className="relative text-xs text-zt-muted">
              ...
              <span className="text-zt-text">
                {formatRelativeTime(ev.at, undefined, locale)}
              </span>{" "}
              — {ev.label}
            </li>
          ))}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-3">
          ...
          <time className="shrink-0 text-xs text-zt-muted">
            {formatRelativeTime(event.at, undefined, locale)}
          </time>
        </div>
        {event.detail ? (
...
export function WeeklyReportView({
  ...
  const { dict } = useDictionary();
  const w = dict.dash.insights.weekly;
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <ReportCard
          icon={Bug}
          label={w.mostCommonErrors}
          items={report.mostCommonErrors}
        />
        <ReportCard
          icon={GitCommitHorizontal}
          label={w.mostUnstableEndpoint}
          items={[report.mostUnstableEndpoint]}
        />
        <ReportCard
          icon={TrendingUp}
          label={w.bestPerformingService}
          items={[report.bestPerformingService]}
        />
        <ReportCard
          icon={TrendingDown}
          label={w.biggestImprovement}
